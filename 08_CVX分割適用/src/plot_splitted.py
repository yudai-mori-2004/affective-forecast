...
def plot_cvx_eda(eda, cvx_segment, name, median_window, median_threshold, output_path, Fs):
    ...
    def get_ylim(data):
        ...
        if max_val <= 1:
            ...
            return int(np.ceil(max_val / 10) * 10)  # 10単位で切り上げ
    ...
    for i, name in enumerate(data_names):
        ...
        if eda_h5 is not None and eda_h5.shape[1] >= 3599:
            ...
            if np.sum(valid_indices) < 2:
                ...
                continue
            x = np.arange(len(eda))
            x_valid = x[valid_indices]      # 有効なデータのインデックス
            eda_valid = eda[valid_indices]  # 有効なデータの値
            eda = np.interp(x, x_valid, eda_valid)  # 全てのxに対して補間
            ...
            # 各時間長でセグメントを切り出してプロット
            lengthes = [900, 840, 780, 720, 660, 600, 540, 480, 420, 360, 300, 240, 180, 120, 60]
            for l in lengthes:
                sid = 3600 - 4*l
                # 全体のCVXEDA結果から該当部分を切り出し
                cvx_segment = {
                    'tonic': tonic_full[sid:],
                    'phasic': phasic_full[sid:]
                }
                output_path = f"/home/mori/projects/affective-forecast/08_CVX分割適用/plots/len_{l}_seconds"
                os.makedirs(output_path, exist_ok=True)
                plot_cvx_eda(eda[sid:], cvx_segment, name, median_window, median_threshold, output_path, Fs)