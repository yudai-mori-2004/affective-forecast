...
if __name__ == "__main__":
    ...
    output_path = f"/home/mori/projects/affective-forecast/plots/EDA分離プロットパターン4"
    ...
    # ガウシアンフィルタのシグマパターン (0.25Hz以上の成分を弱める)
    sigma_patterns = [0.6, 1.0, 1.6]

    for sigma_sec in sigma_patterns:
        for i, name in enumerate(sample_data_names):
            ...
            if eda is not None and eda.shape[1] > 3000:
                ...
                # 2. ガウシアンフィルタ
                y_gaussian = gaussian_filter1d(y_threshold, sigma=sigma_samp, mode="nearest")

                # 3. cvxEDAで分離
                signals = nk.eda_phasic(y_gaussian, sampling_rate=Fs, method="cvxeda")
                ...
                plt.plot(x, y_gaussian, linewidth=0.5, linestyle='--', label="Filtered EDA (Threshold + Gaussian)")
                ...
                plt.title(f"Pattern 4: Threshold + Gaussian + cvxEDA (0.1-25μS, σ={sigma_sec}s, {name})")

                plt.tight_layout()
                plt.savefig(f"{output_path}/{name}/pattern4_sigma_{sigma_sec}s.svg")
                plt.close()

                print(f"Saved Pattern 4 - σ={sigma_sec}s - {i} images")