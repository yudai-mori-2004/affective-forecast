...
if __name__ == "__main__":
    ...
    output_path = f"/home/mori/projects/affective-forecast/plots/EDA分離プロットパターン3"
    ...
    for i, name in enumerate(sample_data_names):
        ...
        if eda is not None and eda.shape[1] > 3000:
            ...
            # 3. cvxEDAで分離
            signals = nk.eda_phasic(y_bandpass, sampling_rate=Fs, method="cvxeda")
            ...
            plt.plot(x, y_bandpass, linewidth=0.5, linestyle='--', label="Filtered EDA (Threshold + Bandpass)")
            ...
            plt.title(f"Pattern 3: Threshold + Bandpass + cvxEDA (0.1-25μS, {name})")

            plt.tight_layout()
            plt.savefig(f"{output_path}/{name}/pattern3.svg")
            plt.close()

            print(f"Saved Pattern 3 - {i} images")