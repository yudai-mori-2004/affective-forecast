...
def load_h5_data(file_path: str, dataset_name: Optional[str] = None) -> np.ndarray:
    ...
    # ファイルパスから新しいフォルダ構造のパスに変換
    filename = os.path.basename(file_path)
    base_dir = "/home/mori/projects/affective-forecast/datas"
    
    # データタイプを判定して適切なフォルダから読み込み
    if "_eda" in filename:
        new_file_path = os.path.join(base_dir, "eda", filename)
    elif "_act" in filename:
        new_file_path = os.path.join(base_dir, "act", filename)
    elif "_rri" in filename:
        new_file_path = os.path.join(base_dir, "rri", filename)
    elif "_temp" in filename:
        new_file_path = os.path.join(base_dir, "temp", filename)
    else:
        # フォールバック: 元のパスを試行
    ...
    if not new_file_path.endswith('.h5'):
        return None
