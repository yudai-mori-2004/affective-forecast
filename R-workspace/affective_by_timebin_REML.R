...
for(bin in all_bins){
  ...
  arousal_model <- lmer(arousal ~ 1 + (1|subject_id), range_dat, REML = TRUE)
  valence_model <- lmer(valence ~ 1 + (1|subject_id), range_dat, REML = TRUE)
...
ggplot(aro_df, aes(x = time_bin, y = mean, group = 1)) +
  ...
  labs(
    x     = "Time Bin",
    y     = "Mean Arousal",
    title = "Arousal ± SD by Time Bin (REML)"
  ) +
  theme(
...
ggplot(val_df, aes(x = time_bin, y = mean, group = 1)) +
  ...
  labs(
    x     = "Time Bin",
    y     = "Mean Valence",
    title = "Valence ± SD by Time Bin  (REML)"
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1)
  )
